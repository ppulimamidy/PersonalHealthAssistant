...
export const healthTwinService = {
  // Get health twin profile (creates one if it doesn't exist)
  async getHealthTwinProfile(): Promise<HealthTwinProfile> {
    const response = await api.get<HealthTwinProfile>('/api/v1/health-twin/profile');
  ...
  async getSimulations(status?: 'draft' | 'active' | 'completed'): Promise<HealthTwinSimulation[]> {
    const url = status
      ? `/api/v1/health-twin/simulations?status=${status}`
      : '/api/v1/health-twin/simulations';
  ...
  async getSimulation(id: string): Promise<HealthTwinSimulation> {
    const response = await api.get<HealthTwinSimulation>(`/api/v1/health-twin/simulations/${id}`);
    return response.data;
  },

  // Create new simulation
  async createSimulation(data: CreateSimulationRequest): Promise<HealthTwinSimulation> {
    const response = await api.post<HealthTwinSimulation>('/api/v1/health-twin/simulations', data);
    return response.data;
  },
  ...
  async updateSimulation(
    ...
    const response = await api.put<HealthTwinSimulation>(`/api/v1/health-twin/simulations/${id}`, data);
    return response.data;
  },

  // Delete simulation
  async deleteSimulation(id: string): Promise<void> {
    await api.delete(`/api/v1/health-twin/simulations/${id}`);
  },

  // Get snapshots
  async getSnapshots(limit?: number): Promise<HealthTwinSnapshot[]> {
    const url = limit
      ? `/api/v1/health-twin/snapshots?limit=${limit}`
      : '/api/v1/health-twin/snapshots';
  ...
  async createSnapshot(notes?: string): Promise<HealthTwinSnapshot> {
    const response = await api.post<HealthTwinSnapshot>('/api/v1/health-twin/snapshots', { notes });
    return response.data;
  },
  ...
  async getHealthTwinGoals(status?: 'active' | 'achieved' | 'abandoned'): Promise<HealthTwinGoal[]> {
    const url = status
      ? `/api/v1/health-twin/goals?status=${status}`
      : '/api/v1/health-twin/goals';
  ...
  async getHealthTwinGoal(id: string): Promise<HealthTwinGoal> {
    const response = await api.get<HealthTwinGoal>(`/api/v1/health-twin/goals/${id}`);
    return response.data;
  },

  // Create new goal
  async createHealthTwinGoal(data: CreateHealthTwinGoalRequest): Promise<HealthTwinGoal> {
    const response = await api.post<HealthTwinGoal>('/api/v1/health-twin/goals', data);
    return response.data;
  },
  ...
  async updateHealthTwinGoal(
    ...
    const response = await api.put<HealthTwinGoal>(`/api/v1/health-twin/goals/${id}`, data);
    return response.data;
  },

  // Delete goal
  async deleteHealthTwinGoal(id: string): Promise<void> {
    await api.delete(`/api/v1/health-twin/goals/${id}`);
  },
  ...
  async updateGoalMilestone(
    ...
    const response = await api.post<HealthTwinGoal>(
      `/api/v1/health-twin/goals/${goalId}/milestones/${milestoneIndex}`,
      { completed }
    );
    return response.data;
  },
