...
class DatabaseSettings(BaseModel):
    ...
    def __init__(self, **data):
        ...
        logger.info(f"DatabaseSettings pool_size: {self.pool_size}, max_overflow: {self.max_overflow}")
    ...
    def __init__(self, **data):
        ...
        if "CORS_ORIGINS" in os.environ:
            ...
            if cors_origins_str.startswith("[") and cors_origins_str.endswith("]"):
                ...
                cors_origins = [origin.strip().strip('"').strip("'") for origin in cors_origins_str[1:-1].split(",")]
...
class Settings(BaseSettings):
    ...
    DOCTOR_COLLABORATION_SERVICE_URL: str = "http://doctor-collaboration.localhost"
            ...
            if "CORS_ORIGINS" in os.environ:
                ...
                if cors_origins_str and cors_origins_str.strip():
                    try:
                        ...
                        if cors_origins_str.startswith("[") and cors_origins_str.endswith("]"):
                            cors_origins = [origin.strip().strip('"').strip("'") for origin in cors_origins_str[1:-1].split(",") if origin.strip()]
                        else:
