"""
Base Agent Class for Health Tracking Agents
Provides common functionality for all autonomous health tracking agents.
"""
...
# Global metrics storage to prevent duplication
_agent_metrics = {}

def get_agent_metrics(agent_name: str) -> Dict[str, Any]:
    """Get or create metrics for an agent (singleton pattern)"""
        ...
        try:
            _agent_metrics[agent_name] = {
                "processing_time": Histogram(
                    ...
                    f"{agent_name} processing time",
                    ["agent_name", "operation"]
                ),
                "success_count": Counter(
                    f"{agent_name}_success_total",
                    f"{agent_name} successful operations",
                    ["agent_name", "operation"]
                ),
                "error_count": Counter(
                    f"{agent_name}_errors_total",
                    f"{agent_name} errors",
                    ["agent_name", "operation", "error_type"]
                ),
                "active_agents": Gauge(
                    f"{agent_name}_active",
                    f"{agent_name} active instances",
                    ["agent_name"]
                )
            }
            logger.info(f"Created metrics for agent: {agent_name}")
        except ValueError as e:
            if "Duplicated timeseries" in str(e):
                logger.warning(f"Metrics for {agent_name} already exist, using existing ones")
                # Create dummy metrics to prevent errors
                _agent_metrics[agent_name] = {
...
class BaseHealthAgent(ABC):
    """
    Base class for all health tracking agents.
    ...
    def __init__(self, agent_name: str, circuit_breaker_config: Optional[Dict[str, Any]] = None):
        ...
        # Get metrics using singleton pattern
        self.metrics = get_agent_metrics(agent_name)
