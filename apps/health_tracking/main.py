...
# Import models
from health_tracking.models.health_metrics import (
    HealthMetric,
    MetricType,
    MetricUnit,
    HealthMetricCreate,
...
def get_metrics():
    ...
    if not _metrics_initialized:
        try:
            ...
            # Guard against duplicate registration on restart (e.g. hot-reload)
            if not getattr(
                prometheus_client.REGISTRY, "_health_tracking_metrics_registered", False
            ):
                _metrics["REQUEST_COUNT"] = Counter(
                    "health_tracking_requests_total",
                    "Total requests",
                    ["method", "endpoint", "status"],
                )
                _metrics["REQUEST_DURATION"] = Histogram(
                    "health_tracking_request_duration_seconds",
                    "Request duration",
                    ["method", "endpoint"],
                )
                _metrics["METRICS_COUNT"] = Gauge(
                    "health_tracking_metrics_total", "Total health metrics stored"
                )
                _metrics["GOALS_COUNT"] = Gauge(
                    "health_tracking_goals_total", "Total health goals"
                )
                _metrics["INSIGHTS_COUNT"] = Gauge(
                    "health_tracking_insights_total", "Total health insights"
                )
                _metrics["VITAL_SIGNS_COUNT"] = Gauge(
                    "health_tracking_vital_signs_total", "Total vital signs"
                )
                _metrics["SYMPTOMS_COUNT"] = Gauge(
                    "health_tracking_symptoms_total", "Total symptoms"
                )
                prometheus_client.REGISTRY._health_tracking_metrics_registered = True
            else:
                # Metrics already registered — retrieve existing collectors
                _metrics[
                    "REQUEST_COUNT"
                ] = prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_requests_total_total"
                ) or prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_requests_total"
                )
                _metrics[
                    "REQUEST_DURATION"
                ...
                _metrics[
                    ...
                    "health_tracking_metrics_total"
                )
                _metrics[
                    "GOALS_COUNT"
                ] = prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_goals_total"
                )
                _metrics[
                    "INSIGHTS_COUNT"
                ] = prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_insights_total"
                )
                _metrics[
                    "VITAL_SIGNS_COUNT"
                ] = prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_vital_signs_total"
                )
                _metrics[
                    "SYMPTOMS_COUNT"
                ] = prometheus_client.REGISTRY._names_to_collectors.get(
                    "health_tracking_symptoms_total"
                )
            _metrics_initialized = True
            logger.info("Prometheus metrics initialized successfully")
        except Exception as e:
            logger.warning(
                f"Failed to initialize Prometheus metrics: {e}, creating dummy metrics"
            )
...
try:
    ...
    configure_opentelemetry(app, "health-tracking-service")
except ImportError:
    pass
...
async def get_agents_health(
    ...
    try:
        ...
        return create_success_response(
            data=agent_health, message="Agent health status retrieved"
        )
...
async def get_dashboard_summary(
    ...
    try:
        ...
        vital_signs_count = await db.execute(
            select(func.count(VitalSigns.id)).where(
        ...
        symptoms_count = await db.execute(
            select(func.count(Symptoms.id)).where(
        ...
        insights_count = await db.execute(
            select(func.count(HealthInsight.id)).where(
