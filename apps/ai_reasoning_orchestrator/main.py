...
from common.middleware.auth import require_auth, get_current_user
from common.models.base import (
    ErrorResponse, SuccessResponse, create_error_response, create_success_response,
    ErrorCode, ErrorSeverity, BaseServiceException
)
...
from .models.reasoning_models import (
    ...
    InsightType, EvidenceSource, ConfidenceLevel
)
...
SERVICE_REGISTRY = {
    "health_tracking": {
        ...
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="health_tracking"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    },
    "medical_records": {
        "url": settings.MEDICAL_RECORDS_SERVICE_URL,
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="medical_records"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    },
    "ai_insights": {
        "url": settings.AI_INSIGHTS_SERVICE_URL,
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="ai_insights"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    },
    "knowledge_graph": {
        "url": settings.KNOWLEDGE_GRAPH_SERVICE_URL,
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="knowledge_graph"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    },
    "nutrition": {
        "url": settings.NUTRITION_SERVICE_URL,
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="nutrition"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    },
    "device_data": {
        "url": settings.DEVICE_DATA_SERVICE_URL,
        "circuit_breaker": CircuitBreaker(failure_threshold=5, recovery_timeout=60, name="device_data"),
        "timeout": TimeoutHandler(timeout=30.0),
        "retry": RetryHandler(max_attempts=3)
    }
}
...
async def reason_about_health(
    ...
    try:
        ...
        logger.info(f"🧠 Processing reasoning request for user {user_id}: {request.query}")
        
        # Step 1: Aggregate user data from all services
        user_data = await app.state.data_aggregator.aggregate_user_data(
            user_id=user_id,
        ...
        knowledge_context = await app.state.knowledge_integrator.get_relevant_knowledge(
            query=request.query,
            user_context=user_data
        )
        
        # Step 3: Generate reasoning and insights
        reasoning_result = await app.state.reasoning_engine.reason(
            query=request.query,
            user_data=user_data,
        ...
        # Step 4: Cache result for future reference (disabled for now due to serialization issues)
        # if redis_client:
        #     cache_key = f"reasoning:{user_id}:{hash(request.query)}"
...
async def natural_language_query(
    ...
    try:
        ...
        # Use the main reasoning endpoint
        return await reason_about_health(reasoning_request, current_user)
...
@app.get("/api/v1/insights/daily-summary")
async def get_daily_insights_summary(
    ...
    try:
        ...
        reasoning_request = ReasoningRequest(
            ...
            time_window="24h",
            data_types=["vitals", "symptoms", "medications", "nutrition", "sleep", "activity"]
        )
...
@app.get("/api/v1/insights/daily-summary-test")
async def get_daily_insights_summary_test():
    """
    ...
    try:
        ...
        # Use the main reasoning endpoint
        return await reason_about_health(reasoning_request, mock_user)
        
    except Exception as e:
        logger.error(f"❌ Daily insights summary test failed: {e}")
        raise HTTPException(status_code=500, detail=f"Daily summary test failed: {str(e)}")
...
async def get_unified_dashboard(
    ...
    """
    try:
        # For now, use a mock user to avoid authentication issues
        # TODO: Implement proper JWT authentication
        current_user = {"id": "test-user", "email": "test@example.com"}
        user_id = current_user["id"]
        ...
        reasoning_request = ReasoningRequest(
            ...
            data_types=["vitals", "symptoms", "medications", "nutrition", "sleep", "activity", "lab_results", "medical_records"]
        )
        
        # Use the main reasoning endpoint
        dashboard_result = await reason_about_health(reasoning_request, current_user)
...
async def get_daily_summary_health(
    ...
    try:
        ...
        # Use the main reasoning endpoint
        daily_result = await reason_about_health(reasoning_request, current_user)
...
async def analyze_symptoms(
    ...
    try:
        ...
        generate_insights = body.get("generate_insights", True)
        ...
        follow_up_questions = await _generate_follow_up_questions(symptoms, query_analysis, include_vitals, include_medications)
        logger.info(f"✅ Generated {len(follow_up_questions)} follow-up questions")
        
        # Create enhanced reasoning request
        reasoning_request = ReasoningRequest(
            query=f"Analyze symptoms with medical context: {', '.join(symptoms)}. Query context: {query_analysis['query_type']}. Severity: {severity}, Duration: {duration}. Additional context: {additional_context}",
            reasoning_type="symptom_analysis",
            time_window="7d",
            data_types=["vitals", "symptoms", "medications", "nutrition", "sleep", "activity", "medical_records", "lab_results"]
        )
        
        # Generate multi-model insights using AI
        multi_model_insights = await _generate_multi_model_insights(symptoms, query_analysis, include_vitals, include_medications)
        
        # Calculate urgency level based on symptoms and context
        urgency_level = await _calculate_urgency_level(symptoms, severity, duration, query_analysis)
        ...
        analysis_result = type('AnalysisResult', (), {
            ...
            'data_sources': ["vitals", "symptoms", "medications", "nutrition", "sleep", "activity", "medical_records", "lab_results"],
            'confidence': "medium"
        })()
...
async def _analyze_symptom_query(symptoms: List[str], context: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if any(word in query_lower for word in ["cause", "causes", "causing", "can cause", "lead to", "why", "reason"]):
            query_analysis["query_type"] = "causation_inquiry"
            query_analysis["intent"] = "understanding_causes"
        elif any(word in query_lower for word in ["diagnosis", "diagnose", "what is", "could be", "condition", "disease"]):
            query_analysis["query_type"] = "diagnostic_inquiry"
            query_analysis["intent"] = "seeking_diagnosis"
        elif any(word in query_lower for word in ["treatment", "cure", "fix", "help", "solution", "remedy"]):
            query_analysis["query_type"] = "treatment_inquiry"
            query_analysis["intent"] = "seeking_treatment"
        elif any(word in query_lower for word in ["weight", "belly", "fat", "diet", "nutrition", "exercise", "calorie", "carb", "carbs", "low carb", "keto"]):
        ...
        if any(word in query_lower for word in ["belly", "stomach", "abdomen", "waist"]):
            medical_entities.append({"entity": "abdominal_fat", "type": "body_composition", "concern": "weight_distribution"})
        if any(word in query_lower for word in ["weight", "fat", "obese", "overweight"]):
            medical_entities.append({"entity": "weight_management", "type": "lifestyle", "concern": "body_composition"})
        if any(word in query_lower for word in ["diet", "nutrition", "eating", "food"]):
            medical_entities.append({"entity": "nutrition", "type": "lifestyle", "concern": "dietary_habits"})
        if any(word in query_lower for word in ["calorie", "calories", "restricted", "low carb", "keto", "carb", "carbs"]):
            medical_entities.append({"entity": "calorie_restriction", "type": "diet", "concern": "metabolic_response"})
        
        # Common symptoms
        if "headache" in query_lower:
            medical_entities.append({"entity": "headache", "type": "symptom", "severity": "variable"})
        if "fatigue" in query_lower or "tired" in query_lower:
            medical_entities.append({"entity": "fatigue", "type": "symptom", "severity": "variable"})
        if "pain" in query_lower:
            medical_entities.append({"entity": "pain", "type": "symptom", "severity": "variable"})
        if "nausea" in query_lower:
            medical_entities.append({"entity": "nausea", "type": "symptom", "severity": "variable"})
        if "dizziness" in query_lower:
            medical_entities.append({"entity": "dizziness", "type": "symptom", "severity": "variable"})
        
        # Metabolic and endocrine
        if any(word in query_lower for word in ["sodium", "salt", "electrolyte"]):
            medical_entities.append({"entity": "sodium", "type": "electrolyte", "concern": "deficiency"})
        if any(word in query_lower for word in ["blood sugar", "glucose", "diabetes"]):
            medical_entities.append({"entity": "blood_sugar", "type": "metabolic", "concern": "regulation"})
        if any(word in query_lower for word in ["thyroid", "hormone"]):
            medical_entities.append({"entity": "thyroid", "type": "endocrine", "concern": "function"})
        
        # Digestive
        if any(word in query_lower for word in ["bloating", "gas", "digestion", "digestive"]):
            medical_entities.append({"entity": "digestive_issues", "type": "symptom", "concern": "gastrointestinal"})
        
        query_analysis["medical_entities"] = medical_entities
        query_analysis["specific_concerns"] = [entity["entity"] for entity in medical_entities]
...
async def _generate_follow_up_questions(symptoms: List[str], query_analysis: Dict[str, Any], include_vitals: bool, include_medications: bool) -> List[Dict[str, Any]]:
    """Generate physician-like follow-up questions dynamically using AI models"""
    try:
        logger.info(f"🤖 Starting AI-powered follow-up question generation for: {symptoms}")
        import openai
        ...
        base_questions = [
            ...
            },
            {
                ...
                "type": "frequency",
                "required": True,
                "options": ["Constant", "Daily", "Several times a week", "Occasionally", "Rarely"]
            }
        ]
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a medical AI assistant that generates relevant follow-up questions for symptom analysis. Always respond with valid JSON."},
            ...
            logger.info(f"✅ Generated {len(ai_questions)} AI-powered follow-up questions for symptoms: {symptom_text}")
            ...
            ai_questions = _generate_dynamic_fallback_questions(symptoms, query_analysis)
        ...
        if include_vitals:
            data_questions.append({
                ...
                "type": "vitals",
                "required": False,
                "options": ["Yes, higher", "Yes, lower", "No changes", "Don't monitor"]
            })
        ...
        if include_medications:
            data_questions.append({
                ...
                "type": "medications",
                "required": False,
                "options": ["Yes, new medication", "Yes, stopped medication", "Yes, changed dose", "No changes"]
            })
...
def _generate_dynamic_fallback_questions(symptoms: List[str], query_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Generate dynamic fallback questions based on symptoms and query analysis"""
    questions = []
    ...
    if any(word in symptom_text for word in ["belly", "stomach", "abdomen", "waist", "fat"]):
        questions.extend([
            {
                ...
                "required": False,
                "options": ["Only belly/waist", "All over body", "Face and neck", "Arms and legs", "Mixed distribution"]
            },
            {
                "id": "diet_consistency",
                ...
                "type": "lifestyle",
                "required": False,
                "options": ["Very strictly", "Mostly consistent", "Sometimes cheat", "Not very strict", "Inconsistent"]
            },
            {
                "id": "exercise_routine",
                ...
                "type": "lifestyle",
                "required": False,
                "options": ["Cardio only", "Strength training", "Both cardio and strength", "No exercise", "Occasional walks"]
            }
        ])
    
    # Diet and nutrition specific questions
    if any(word in symptom_text for word in ["diet", "nutrition", "eating", "calorie", "low carb"]):
        questions.extend([
            {
                "id": "diet_type",
                ...
                "type": "nutrition",
                "required": False,
                "options": ["Low carb/keto", "Calorie restriction", "Intermittent fasting", "Mediterranean", "Other"]
            },
            {
                "id": "meal_timing",
                ...
                "type": "nutrition",
                "required": False,
                "options": ["1-2 meals", "3 meals", "4-5 small meals", "Grazing throughout day", "Intermittent fasting"]
            },
            {
                "id": "water_intake",
                ...
                "type": "nutrition",
                "required": False,
                "options": ["Less than 4 cups", "4-6 cups", "6-8 cups", "More than 8 cups", "Don't track"]
            }
        ])
    
    # Metabolic and endocrine questions
    if any(word in symptom_text for word in ["metabolism", "thyroid", "hormone", "blood sugar"]):
        questions.extend([
            {
                "id": "energy_levels",
    ...
    # Digestive questions
    if any(word in symptom_text for word in ["bloating", "gas", "digestion", "digestive", "stomach"]):
        questions.extend([
            {
                ...
                "type": "digestive",
                "required": False,
                "options": ["Bloating", "Gas", "Indigestion", "Nausea", "None of the above"]
            },
            {
                "id": "food_triggers",
                ...
                "type": "digestive",
                "required": False,
                "options": ["Dairy", "Gluten", "High fat foods", "Spicy foods", "No specific triggers"]
            }
        ])
        ...
        questions.extend([
            {
                ...
                "type": "lifestyle",
                "required": False,
                "options": ["Weight loss", "Better health", "More energy", "Better sleep", "Other"]
            },
            {
                ...
                "type": "lifestyle",
                "required": False,
                "options": ["Yes, worked well", "Yes, some success", "Yes, no success", "No previous attempts", "Not sure"]
            }
        ])
    
    return questions

def _generate_dynamic_fallback_insights(symptoms: List[str], query_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if any(word in symptom_text for word in ["belly", "stomach", "abdomen", "waist", "fat"]):
        ...
        medical_context["key_factors"].extend(["Hormonal balance", "Insulin sensitivity", "Stress levels"])
        medical_context["risk_factors"].extend(["High stress", "Poor sleep", "Sedentary lifestyle"])
        medical_context["diagnostic_considerations"].extend(["Hormone levels", "Insulin resistance", "Metabolic health"])
    ...
    if any(word in symptom_text for word in ["diet", "nutrition", "eating", "calorie", "low carb"]):
        ...
        medical_context["key_factors"].extend(["Protein intake", "Calorie deficit", "Nutrient timing"])
        medical_context["risk_factors"].extend(["Very low calorie diets", "Nutrient deficiencies"])
        medical_context["diagnostic_considerations"].extend(["Body composition", "Metabolic rate", "Nutrient status"])
    
    # Exercise and lifestyle insights
    if any(word in symptom_text for word in ["exercise", "workout", "activity", "training"]):
        insights.append({
            "type": "lifestyle",
        ...
        medical_context["key_factors"].extend(["Exercise type", "Intensity", "Consistency"])
        medical_context["risk_factors"].extend(["Sedentary lifestyle", "Overtraining"])
        medical_context["diagnostic_considerations"].extend(["Fitness level", "Recovery capacity"])
    ...
    if query_type == "lifestyle_inquiry":
        ...
        medical_context["key_factors"].extend(["Sleep quality", "Stress management", "Consistency"])
        medical_context["risk_factors"].extend(["Poor sleep", "High stress", "Inconsistent habits"])
        medical_context["diagnostic_considerations"].extend(["Lifestyle factors", "Behavioral patterns"])
...
async def _generate_multi_model_insights(symptoms: List[str], query_analysis: Dict[str, Any], include_vitals: bool, include_medications: bool) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a medical AI assistant that provides evidence-based medical insights and analysis. Always respond with valid JSON."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.2,
                max_tokens=1500
            ...
            # Generate dynamic fallback insights based on symptoms and query analysis
            fallback_data = _generate_dynamic_fallback_insights(symptoms, query_analysis)
...
async def _calculate_urgency_level(symptoms: List[str], severity: str, duration: str, query_analysis: Dict[str, Any]) -> Dict[str, Any]:
    """Calculate urgency level based on symptoms and context"""
    try:
        query_text = " ".join(symptoms).lower()
        urgency_level = "low"
        reasoning = "Based on current information, this appears to be a non-urgent situation."
        next_steps = [
        ...
        if "sunrash" in query_text or "sun rash" in query_text or "sunburn" in query_text:
            if "severe" in query_text or "blisters" in query_text or "fever" in query_text:
        ...
        elif "heartburn" in query_text or "acid reflux" in query_text or "indigestion" in query_text:
            if "severe" in query_text or "chest pain" in query_text or "difficulty swallowing" in query_text:
                urgency_level = "medium"
            ...
            urgent_headache_symptoms = ["severe headache", "sudden headache", "worst headache", "thunderclap headache", "headache with fever", "headache with confusion"]
            if any(urgent_symptom in query_text for urgent_symptom in urgent_headache_symptoms):
                urgency_level = "high"
            ...
            if "high fever" in query_text or "above 103" in query_text or "fever with confusion" in query_text:
                urgency_level = "high"
            ...
            elif "fever" in query_text and ("persistent" in query_text or "more than 3 days" in query_text):
                urgency_level = "medium"
            ...
            if "blood" in query_text or "shortness of breath" in query_text or "chest pain" in query_text:
                urgency_level = "high"
            ...
            if "severe pain" in query_text or "sudden pain" in query_text or "chest pain" in query_text:
                urgency_level = "high"
            ...
            if "severe rash" in query_text or "blisters" in query_text or "fever with rash" in query_text:
                urgency_level = "medium"
            ...
            if "severe nausea" in query_text or "blood in vomit" in query_text or "severe abdominal pain" in query_text:
                urgency_level = "high"
            ...
            elif "persistent vomiting" in query_text or "more than 24 hours" in query_text:
                urgency_level = "medium"
            ...
            if "severe fatigue" in query_text or "extreme fatigue" in query_text or "fatigue with other symptoms" in query_text:
                urgency_level = "medium"
                ...
                urgency_level = "low"
                reasoning = "Mild fatigue is common and often related to lifestyle factors."
                next_steps = [
...
async def _generate_personalized_recommendations(symptoms: List[str], query_analysis: Dict[str, Any], urgency_level: Dict[str, Any], include_vitals: bool, include_medications: bool) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a medical AI assistant that generates personalized health recommendations. Always respond with valid JSON."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=1000
            ...
            logger.info(f"Generated {len(ai_recommendations)} AI-powered recommendations for symptoms: {symptom_text}")
        ...
        if urgency_level["level"] == "high":
            ...
            urgency_recommendations.append({
                ...
                "priority": "high",
                "actionable": True,
                "evidence": ["Medical evaluation can help identify underlying causes"],
                "follow_up": "Schedule appointment within 1-2 weeks"
            })
        
        # Combine all recommendations
        all_recommendations = base_recommendations + ai_recommendations + urgency_recommendations
...
async def health_query(
    ...
    try:
        ...
        # Use the main reasoning endpoint
        query_result = await reason_about_health(reasoning_request, current_user)
        ...
        should_redirect_to_symptoms = query_analysis.get("should_redirect_to_symptoms", False)
...
async def _analyze_health_query(query: str, context: str) -> Dict[str, Any]:
    ...
    try:
        ...
        symptom_keywords = ["i have", "i'm experiencing", "i feel", "my", "suffering from", "having", "experiencing"]
        if any(keyword in query_lower for keyword in symptom_keywords):
            ...
            query_analysis["redirect_reason"] = "This appears to be a symptom-specific query that would benefit from our comprehensive symptom analysis"
            query_analysis["intent"] = "symptom_assessment"
        
        # Check for specific query types
        elif any(word in query_lower for word in ["symptoms of", "signs of", "what are the symptoms"]):
            query_analysis["query_type"] = "symptom_education"
            query_analysis["category"] = "education"
            query_analysis["intent"] = "learning_symptoms"
        elif any(word in query_lower for word in ["cause", "causes", "why", "what causes"]):
            query_analysis["query_type"] = "causation_education"
            query_analysis["category"] = "education"
            query_analysis["intent"] = "understanding_causes"
        elif any(word in query_lower for word in ["treatment", "cure", "how to treat", "remedy"]):
            query_analysis["query_type"] = "treatment_education"
            query_analysis["category"] = "education"
            query_analysis["intent"] = "seeking_treatment_info"
        elif any(word in query_lower for word in ["normal", "range", "what is normal", "healthy"]):
        ...
        if "diabetes" in query_lower:
            medical_entities.append({"entity": "diabetes", "type": "condition", "category": "metabolic"})
        if "blood pressure" in query_lower or "hypertension" in query_lower:
            medical_entities.append({"entity": "blood_pressure", "type": "vital", "category": "cardiovascular"})
        if "heart rate" in query_lower or "pulse" in query_lower:
            medical_entities.append({"entity": "heart_rate", "type": "vital", "category": "cardiovascular"})
        if "headache" in query_lower:
            medical_entities.append({"entity": "headache", "type": "symptom", "category": "neurological"})
...
async def _generate_educational_content(query: str, query_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if query.strip():  # Only generate questions if there's an actual query
            try:
                ...
                symptoms = [query]  # Treat the query as a symptom for question generation
                include_vitals = True
                include_medications = True
                
                # Generate dynamic questions
                dynamic_questions = await _generate_follow_up_questions(symptoms, query_analysis, include_vitals, include_medications)
                
                # Convert the structured questions to simple text format for educational content
                educational_content["follow_up_questions"] = [
                    question["question"] for question in dynamic_questions[:5]  # Limit to 5 questions
                ]
                
                dynamic_questions_generated = True
                logger.info(f"Generated {len(educational_content['follow_up_questions'])} dynamic questions for query: {query}")
...
async def doctor_report(
    ...
    try:
        ...
        logger.info(f"🔄 Generating doctor report for user {user_id}, type: {report_type}")
        ...
        # Use the main reasoning endpoint
        report_result = await reason_about_health(reasoning_request, current_user)
        ...
        raise HTTPException(status_code=500, detail=f"Doctor report generation failed: {str(e)}")

# Test endpoint for unified dashboard without authentication
@app.get("/health/unified-dashboard-test")
async def get_unified_dashboard_test():
    """
    Test endpoint for unified dashboard without authentication.
    """
    try:
        ...
        # Use the main reasoning endpoint
        dashboard_result = await reason_about_health(reasoning_request, mock_user)
        ...
        raise HTTPException(status_code=500, detail=f"Dashboard test failed: {str(e)}")

# Doctor mode report endpoint
@app.post("/api/v1/doctor-mode/report")
async def generate_doctor_report(
    ...
    try:
        ...
        reasoning_request = ReasoningRequest(
            ...
            data_types=["vitals", "symptoms", "medications", "nutrition", "sleep", "activity", "lab_results"]
        )
        
        # Use the main reasoning endpoint
        reasoning_result = await reason_about_health(reasoning_request, current_user)
        ...
        return create_success_response(
            data=doctor_report,
            message="Doctor mode report generated successfully"
        )
...
@app.websocket("/ws/insights/{user_id}")
async def websocket_insights(websocket: WebSocket, user_id: str):
    ...
    try:
        while True:
            ...
            reasoning_request = ReasoningRequest(
                query="Generate real-time health insights",
                reasoning_type="real_time_insights",
                time_window="1h",
                data_types=["vitals", "symptoms", "alerts"]
            )
            ...
            await websocket.send_text(insight_data)
