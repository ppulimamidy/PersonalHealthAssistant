...
async def readiness_check() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            conn = await asyncpg.connect('postgresql://postgres:your-super-secret-and-long-postgres-password@supabase-db:5432/postgres')
...
for router_name, router_path in additional_routers:
    try:
        module = __import__(router_path, fromlist=["router"])
        if hasattr(module, "router"):
            app.include_router(module.router, prefix=f"/{router_name}", tags=[router_name.title()])
            logger.info(f"{router_name.title()} router included successfully")
    except ImportError:
        logger.debug(f"Router {router_name} not found, skipping")
    except Exception as e:
        logger.warning(f"Could not include {router_name} router: {e}") 