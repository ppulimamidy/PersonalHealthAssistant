...
class AuthService(BaseService):
    ...
    def verify_password(self, plain_password: str, hashed_password: str) -> bool:
        ...
        logger.warning(f"[DEBUG] Login: plain_password='{plain_password}' hashed_password='{hashed_password}'")
    ...
    def create_access_token(self, data: dict, expires_delta: Optional[timedelta] = None) -> str:
        ...
        encoded_jwt = jwt.encode(to_encode, self.get_jwt_secret_key(), algorithm=JWT_ALGORITHM)
        return encoded_jwt
    ...
    def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            payload = jwt.decode(token, self.get_jwt_secret_key(), algorithms=[JWT_ALGORITHM])
            return payload
        except JWTError:
            return None
    ...
    async def authenticate_user(self, email: str, password: str, ip_address: str = None, user_agent: str = None) -> Optional[User]:
        ...
        try:
            ...
            if not user:
                ...
                await self._log_failed_login(email, ip_address, user_agent, "user_not_found")
                return None
            ...
            if not self.verify_password(password, user.password_hash):
                ...
                await self._log_failed_login(email, ip_address, user_agent, "invalid_password")
                return None
            
            if not user.is_active:
                logger.warning(f"User inactive: {email}")
                await self._log_failed_login(email, ip_address, user_agent, "account_inactive")
                return None
            
            if user.is_locked:
                logger.warning(f"User locked: {email}")
                await self._log_failed_login(email, ip_address, user_agent, "account_locked")
                return None
    ...
    async def authenticate_with_supabase(self, supabase_token: str, ip_address: str = None, user_agent: str = None) -> Optional[User]:
        """Authenticate user with Supabase token."""
        try:
            # Verify token with Supabase
            supabase_user = await self.supabase_service.verify_token(supabase_token)
            if not supabase_user:
                await self._log_failed_login("supabase_user", ip_address, user_agent, "invalid_supabase_token")
                return None
            
            # Get or create user in our database
            user = await self._get_or_create_supabase_user(supabase_user)
            if not user.is_active:
                await self._log_failed_login(user.email, ip_address, user_agent, "account_inactive")
                return None
            ...
            await self._log_failed_login("supabase_user", ip_address, user_agent, "supabase_error")
            return None
    
    async def authenticate_with_auth0(self, auth0_token: str, ip_address: str = None, user_agent: str = None) -> Optional[User]:
        """Authenticate user with Auth0 token."""
        try:
            # Verify token with Auth0
            auth0_user = await self.auth0_service.verify_token(auth0_token)
            if not auth0_user:
                await self._log_failed_login("auth0_user", ip_address, user_agent, "invalid_auth0_token")
                return None
            ...
            await self._log_failed_login("auth0_user", ip_address, user_agent, "auth0_error")
            return None
    
    async def create_session(self, user: User, ip_address: str = None, user_agent: str = None, device_id: str = None) -> Session:
        ...
        try:
            ...
            session = Session(
                ...
                access_token_expires_at=datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES),
    ...
    async def refresh_session(self, refresh_token: str, ip_address: str = None, user_agent: str = None) -> Optional[Session]:
        ...
        try:
            ...
            session.access_token_expires_at = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
            session.refresh_token_expires_at = datetime.utcnow() + timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)
    ...
    async def revoke_session(self, session_id: UUID, user_id: UUID, reason: str = "manual_logout") -> bool:
    ...
    async def verify_mfa(self, user: User, mfa_code: str, session_id: UUID = None) -> bool:
    ...
    async def setup_mfa(self, user: User, device_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
                name=user.email,
                issuer_name="Personal Health Assistant"
            )
            ...
            backup_codes = await self.mfa_service.generate_backup_codes(user.id, device.id)
    ...
    async def _log_successful_login(self, user: User, ip_address: str = None, user_agent: str = None):
    ...
    async def _log_failed_login(self, email: str, ip_address: str = None, user_agent: str = None, reason: str = "unknown"):
    ...
    async def request_password_reset(self, email: str, ip_address: str = None, user_agent: str = None) -> bool:
        ...
        try:
            ...
            user.user_metadata["password_reset_expires"] = (datetime.utcnow() + timedelta(hours=1)).isoformat()
    ...
    async def confirm_password_reset(self, email: str, token: str, new_password: str, ip_address: str = None, user_agent: str = None) -> bool:
        ...
        try:
            ...
            if not user:
                logger.warning(f"Password reset confirmation for non-existent user: {email}")
                return False
    ...
    async def request_email_verification(self, email: str, ip_address: str = None, user_agent: str = None) -> bool:
        ...
        try:
            ...
            user.user_metadata["email_verification_expires"] = (datetime.utcnow() + timedelta(hours=24)).isoformat()
    ...
    async def confirm_email_verification(self, email: str, token: str, ip_address: str = None, user_agent: str = None) -> bool:
        ...
        try:
            ...
            if not user:
                logger.warning(f"Email verification confirmation for non-existent user: {email}")
                return False
            ...
            user.status = UserStatus.ACTIVE  # Activate account if it was pending verification
            user.user_metadata = user.user_metadata or {}
    ...
    async def _send_email_verification_email(self, email: str, token: str):
        ...
        # TODO: Implement email service integration
        # For now, just log the token (in production, send actual email)
        logger.info(f"Email verification email would be sent to {email} with token: {token}")
        # In production, you would use a service like SendGrid, AWS SES, etc.
    
    async def _log_audit_event(self, event_type: str, user_id: Optional[UUID], ip_address: str = None, user_agent: str = None, metadata: Dict[str, Any] = None):
        """Log audit event."""
        try:
            await self.audit_service.log_event(
