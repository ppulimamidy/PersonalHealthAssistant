...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            logger.info(
                "Health Analysis Service initialized without AI models (lazy loading enabled)"
            )
...
setup_prometheus_metrics(app, service_name="health-analysis-service")
...
# Include routers
app.include_router(
    health_analysis.router, prefix="/api/v1/health-analysis", tags=["Health Analysis"]
)
...
async def health_check():
    ...
    try:
        ...
        service_health = (
            await health_analysis_service.get_health_status()
            if health_analysis_service
