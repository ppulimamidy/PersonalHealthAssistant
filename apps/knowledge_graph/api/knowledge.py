...
from apps.knowledge_graph.models.knowledge_models import (
    ...
    SemanticSearchQuery, PathQuery, RecommendationQuery,
...
from apps.knowledge_graph.services.knowledge_graph_service import KnowledgeGraphService, get_knowledge_graph_service
...
async def list_medical_entities(
    ...
    source: Optional[OntologySource] = Query(None, description="Filter by ontology source"),
    ...
    try:
        logger.info(f"Listing medical entities with filters: type={entity_type}, limit={limit}")
...
async def create_relationship(
    ...
    try:
        logger.info(f"Creating relationship between {relationship_data.source_entity_id} and {relationship_data.target_entity_id}")
...
async def find_paths(
    ...
    try:
        logger.info(f"Finding paths between {query.source_entity_id} and {query.target_entity_id}")
...
@router.get("/recommendations/patient/{patient_id}", response_model=List[MedicalRecommendation])
@security_headers
...
async def get_statistics(
    ...
    try:
        ...
        logger.info(f"Retrieved statistics: {stats.total_entities} entities, {stats.total_relationships} relationships")
...
async def import_ontology(
    ...
    try:
        ...
        # Placeholder implementation
        import_response = OntologyImportResponse(
        ...
        logger.info(f"Ontology import completed: {import_response.entities_imported} entities, {import_response.relationships_imported} relationships")
    ...
    try:
        ...
        insights = [
            HealthInsight(
                ...
                description="Based on your health profile, you have moderate cardiovascular risk factors.",
                confidence=ConfidenceLevel.MEDIUM,
                evidence=["Blood pressure readings", "Family history", "Lifestyle factors"],
                related_conditions=[],
                related_symptoms=[],
                risk_factors=[],
                recommendations=["Monitor blood pressure regularly", "Maintain healthy diet", "Exercise regularly"],
                created_at=datetime.utcnow()
            )
        ]
