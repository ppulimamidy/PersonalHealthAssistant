...
from ..models.profile import ProfileCreate, ProfileUpdate, ProfileResponse, ProfileSummary
...
async def import_profile_data(
    ...
    """
    try:
        profile = await profile_service.import_profile_data(current_user.id, import_data)
...
async def get_profile_completion(
    ...
    try:
        ...
        logger.error(f"Failed to get profile completion for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
