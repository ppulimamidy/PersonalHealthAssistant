...
class FoodRecognitionResult(Base):
    ...
    # Relationships
    corrections = relationship("UserCorrection", back_populates="recognition_result", cascade="all, delete-orphan")
...
class UserCorrection(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    recognition_result_id = Column(UUID(as_uuid=True), ForeignKey('nutrition.food_recognition_results.id'), nullable=False)
    user_id = Column(String(50), nullable=False, index=True)
    timestamp = Column(DateTime, default=datetime.utcnow, nullable=False)
    ...
    # Relationships
    recognition_result = relationship("FoodRecognitionResult", back_populates="corrections")
...
class MealLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(String(50), nullable=False, index=True)
    recognition_result_id = Column(UUID(as_uuid=True), ForeignKey('nutrition.food_recognition_results.id'), nullable=True)
    timestamp = Column(DateTime, default=datetime.utcnow, nullable=False)
    ...
    # Relationships
    recognition_result = relationship("FoodRecognitionResult", back_populates="meal_logs")
    ...
    __table_args__ = (
        ...
        Index('idx_meal_type', 'meal_type'),
        Index('idx_recognition_result', 'recognition_result_id'),
        Index('idx_synced_flags', 'synced_to_health_tracking', 'synced_to_medical_analysis'),
        {'schema': 'nutrition'}
    )
...
class NutritionGoal(Base):
    ...
    goal_type = Column(String(30), nullable=False)  # weight_loss, muscle_gain, maintenance, health_improvement
...
class FoodDatabase(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_access_count', 'access_count', postgresql_ops={'access_count': 'DESC'}),
        Index('idx_last_accessed', 'last_accessed', postgresql_ops={'last_accessed': 'DESC'}),
        {'schema': 'nutrition'}
    )
...
class UserPreferences(Base):
    ...
    preferred_recognition_models = Column(JSON, nullable=True)  # Array of preferred models
