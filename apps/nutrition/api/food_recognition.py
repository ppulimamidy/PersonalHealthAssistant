...
# pylint: disable=too-many-locals,too-many-branches,too-many-statements,broad-except,too-few-public-methods,invalid-name,line-too-long,too-many-lines,too-many-arguments,logging-fstring-interpolation,raise-missing-from,unexpected-keyword-arg,no-value-for-parameter,missing-function-docstring
...
from apps.nutrition.models.food_recognition_models import (
    ...
    BatchFoodRecognitionRequest,
    BatchFoodRecognitionResponse,
    RecognizedFood,
    RecognitionModel,
)
...
async def recognize_food_from_image(
    ...
    # Validate image format before sending to service
    ...
    if image_ext not in ("jpg", "jpeg", "png", "gif", "webp", "heic"):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Unsupported image format '{image_ext}'. Accepted: jpg, png, webp, gif, heic.",
        )
    ...
    try:
        ...
        model_list = [
            RecognitionModel(model.strip())
            for model in (models_to_use or "google_vision").split(",")
        ]
        restrictions = (
        ...
        logger.warning(f"Invalid input for food recognition: {e}")
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=str(e),
        )
        ...
        if e.status == 429:
            raise HTTPException(
                ...
                detail="Vision API rate limit exceeded. Please try again in a moment.",
            )
        logger.error(f"Vision API HTTP error {e.status} for user {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=f"Vision service returned an error ({e.status}). Please try again.",
        )
    except aiohttp.ClientError as e:
        logger.error(f"Vision API connection error for user {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Could not reach the vision service. Please try again.",
        )
...
@food_recognition_router.post(
    "/recognize-batch", response_model=BatchFoodRecognitionResponse
)
async def recognize_food_batch(
    ...
    try:
        ...
        logger.info(
            f"Processing batch food recognition for user {batch_request.user_id}"
        )
        ...
        logger.info(
            f"Batch food recognition completed for user {batch_request.user_id}"
        )
        return batch_result
...
async def correct_food_recognition(
    ...
    try:
        ...
        updated_result = await recognition_service.correct_recognition(
            recognition_id=recognition_id, user_id=user_id, corrections=corrections
        )
...
async def improve_recognition_model(
    ...
    try:
        ...
        result = await recognition_service.submit_training_data(
            user_id=user_id, training_data=training_data
        )
...
async def delete_recognition_result(
    recognition_id: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        result = await recognition_service.delete_recognition_result(
            recognition_id=recognition_id, user_id=user_id
        )
...
@food_recognition_router.post(
    "/analyze-nutrition", response_model=FoodRecognitionResponse
)
async def analyze_food_nutrition(
    ...
    try:
        ...
        if enable_detailed_analysis:
            analysis_result[
                "nutrition_insights"
            ] = _generate_detailed_nutrition_insights(analysis_result)
        ...
        logger.warning(f"Invalid input for nutrition analysis: {e}")
        raise HTTPException(
...
def _generate_detailed_nutrition_insights(
    ...
    daily_values = {
        ...
        "fiber": sum(
            food.get("fiber_g", 0)
            for food in analysis_result.get("recognized_foods", [])
        )
        / 28
        * 100,  # 28g daily value
        "sodium": sum(
    ...
    return {
        ...
        "meal_assessment": {
            ...
            "suitable_for": _determine_meal_suitability(
...
def _determine_meal_suitability(
...
def _calculate_nutrition_score(analysis_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if total_macros > 0:
        ...
        if (
            0.1 <= protein_ratio <= 0.35
            and 0.3 <= carbs_ratio <= 0.7
            and 0.1 <= fat_ratio <= 0.45
        ):
            score += 25
        elif (
            0.05 <= protein_ratio <= 0.4
            and 0.2 <= carbs_ratio <= 0.8
            and 0.05 <= fat_ratio <= 0.5
        ):
            score += 20
        else:
            score += 10
